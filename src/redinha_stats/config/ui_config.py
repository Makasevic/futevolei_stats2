"""Configuracoes de marca e parametros de interface customizaveis."""
...
class NavigationLink:
    """Link de navegacao exibido no topo da pagina."""
...
class BrandingConfig:
    ...
    hero_description: str = (
        "Aqui mora a verdade: sai o bla-bla-bla, entram os numeros."
...
@dataclass(frozen=True)
class UiConfig:
    """Parametros ajustaveis da interface HTML/Flask."""
...
def _normalize_tuple(
    values: Iterable[str] | str | None, *, default: Tuple[str, ...]
) -> Tuple[str, ...]:
    if values is None:
        return default
...
def build_ui_config(overrides: Mapping[str, Any] | None = None) -> UiConfig:
    """Cria uma instancia de :class:`UiConfig` mesclando *overrides* opcionais."""
...
def get_ui_config(overrides: Mapping[str, Any] | None = None) -> UiConfig:
    """Retorna a configuracao imutavel de UI usada pelo Flask."""
