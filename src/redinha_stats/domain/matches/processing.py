...
def filtrar_dados(df: pd.DataFrame, modo: str, valor: str) -> pd.DataFrame:
    ...
    if modo_normalizado == "Dias":
        indice = df.index
        tzinfo = getattr(indice, "tz", None) if isinstance(indice, pd.DatetimeIndex) else None
        hoje = pd.Timestamp.now(tz=tzinfo) if tzinfo is not None else pd.Timestamp.now()
...
def preparar_dados_individuais(df: pd.DataFrame) -> pd.DataFrame:
    """Calcula vitorias, derrotas, score (%) e rank para cada jogador."""
...
def preparar_dados_duplas(df: pd.DataFrame) -> pd.DataFrame:
    """Prepara dados de vitorias, derrotas e score para duplas."""
