...
def criar_colunas_duplas(df):
    """
    Cria colunas para duplas vencedoras e perdedoras, garantindo consistencia de ordenacao.
    ...
    df["dupla_winner"] = df.apply(
        lambda row: " e ".join(sorted([row["winner1"], row["winner2"]])), axis=1
    )
    df["dupla_loser"] = df.apply(
        lambda row: " e ".join(sorted([row["loser1"], row["loser2"]])), axis=1
    )
